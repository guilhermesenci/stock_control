...
function onSearch() {
  ...
  try {
    ...
    const convertedFilters = {
      ...filters.value,
      transactionsDateFrom: formatBrazilianDateToISO(filters.value.transactionsDateFrom),
      transactionsDateTo: formatBrazilianDateToISO(filters.value.transactionsDateTo),
    }
    // First update the model
    emit('update:modelValue', convertedFilters)
    console.log('TransactionsFilters: Converted filters for emission:', convertedFilters)
