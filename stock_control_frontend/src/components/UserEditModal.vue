<!-- UserEditModal.vue - Refatorado com novo sistema UX -->
<template>
        ...
        <div class="form-fields">
          ...
          <div class="form-field">
            <label class="checkbox-label">
              ...
              />
              <span class="checkbox-text">Usuário Master</span>
            </label>
          </div>
...
// Regras de validação para senha (opcional)
const passwordRules = computed(() => {
  if (!formData.value.password) {
    return []; // Senha é opcional
  }
  return [
...
// Watch para limpar password2 quando password for limpo
...
const handleSubmit = async () => {
  ...
  try {
    await withLoading(async () => {
      // Remove senhas vazias antes de enviar
      const userData: UpdateUserData = { ...formData.value };
